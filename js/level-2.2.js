...
function mouseDown(e) {
  ...
  if (e.target.classList.contains("block")) {
    ...
    if (selectedBlock.classList[1].slice(6) > 4 && selectedBlock.parentElement.classList.contains("validation")) {
  ...
  if (selectedBlock) {
    ...
    if (selectedBlock.classList[1].slice(6) > 4) {
      if (selectedBlock.parentElement === menu) {
        if (elementUnderMouse === null || !elementUnderMouse.parentElement.classList.contains("validation") && elementUnderMouse !== allBlocks && elementUnderMouse.parentElement !== allBlocks) {
          blocks.forEach((block) => {
    ...
    blocks.forEach((block) => {
      if (block !== selectedBlock) {
        ...
        if (result.isOverlapping) {
          if (selectedBlock.classList[1].slice(6) > 4) {
            if (block.classList.contains("validation")) {
...
function checkSorting(parent) {
  ...
  if (parent.classList.contains("validation")) {
    times = 10;
