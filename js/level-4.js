  ...
  function animateAndCheck() {
    ...
    if (currentBlock) {
      ...
      if (parent === allBlocks) {
        ...
        checkSorting(parent);
        return;
      } else if (parent.classList.contains("block-4") && children.length === 4) {
...
function checkBlockOverlap(block1, block2, offset) {
  ...
  if (isOverlapping) {
    if (offset === "x") {
      ...
      if (rect1.bottom > rect2.top && rect1.bottom < rect2.bottom) {
        overlapType = 'fromTop';
      } else if (rect1.top < rect2.bottom && rect1.top > rect2.top) {
        overlapType = 'fromBottom';
      }
    }
  }
